...
try {
  ...
  assert.equal(await panel.locator('[data-mission]').count(),12);assert.equal(await row(107).count(),0);assert.equal(await row(9).count(),0);assert.equal(await row(113).count(),1);
  ...
  await panel.getByLabel('Mission value').selectOption('3');assert.deepEqual(await panel.locator('[data-mission]').evaluateAll(nodes=>nodes.map(n=>n.dataset.mission)),['102']);
  await panel.getByLabel('Mission value').selectOption('all');assert.match(await row(106).innerText(),/Value unavailable/);
  ...
  await page.screenshot({path:'audit/alliance-57-desktop.png'});
  ...
  await support(108).click();await idle();assert.match(await row(108).innerText(),/No available Fire Officer/);await support(109).click();await idle();assert.match(await row(109).innerText(),/not reported officer travel times/);assert.equal(report.dispatches.length,3);
  ...
  await panel.locator('.nx-as-list').evaluate(n=>n.scrollTop=n.scrollHeight);assert.ok(await panel.getByRole('button',{name:'Support selected',exact:true}).isVisible());await page.screenshot({path:'audit/alliance-57-phone.png'});
