...
fs.mkdirSync('audit/audit77-large-popup-button',{recursive:true});
...
try {
  ...
  await page.locator('#mission-update-box').waitFor();
  const dialogs=[];topPage.on('dialog',async d=>{dialogs.push(d.message());await d.dismiss();});

  await page.getByRole('button',{name:'Auto Mode: Start',exact:true}).click();
  await page.waitForFunction(()=>!!document.querySelector('#mf-auto-stop-reason')?.textContent || document.querySelectorAll('input.vehicle_checkbox:checked').length>=133,{},{timeout:25000});
  for(let attempt=0;attempt<50&&!report.dispatches.length;attempt++)await page.waitForTimeout(100);
  assert.equal(report.dispatches.length,1);assert.equal(report.dispatches[0].selected.length,133);
  const outcome=await page.evaluate(()=>({selected:[...document.querySelectorAll('input.vehicle_checkbox:checked')].map(x=>x.value),status:document.querySelector('#status-box-message').textContent,stopReason:document.querySelector('#mf-auto-stop-reason')?.textContent}));
  console.log(JSON.stringify({outcome,errors:report.errors}));
  ...
  fs.writeFileSync('audit/auto-reconcile-81-outcome.json',JSON.stringify({outcome,errors:report.errors,realGame:false},null,2));
  console.log('PASS actual Auto Mode selects all 133 mixed units despite stale patient ledger');
