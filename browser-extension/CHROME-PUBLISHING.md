...
The configured provider is `projects/809679386301/locations/global/workloadIdentityPools/nexus-github/providers/github-main`. It maps `google.subject=assertion.sub`. Its condition requires repository ID `1305627141`, repository `Team-Killing-Bastards/MissionChief-Command-Nexus`, ref `refs/heads/main`, workflow `Team-Killing-Bastards/MissionChief-Command-Nexus/.github/workflows/edge-extension.yml@refs/heads/main`, and excludes `pull_request` and `pull_request_target` events.

The principal `principal://iam.googleapis.com/projects/809679386301/locations/global/workloadIdentityPools/nexus-github/subject/repo:Team-Killing-Bastards@306750767/MissionChief-Command-Nexus@1305627141:ref:refs/heads/main` has `roles/iam.workloadIdentityUser` on that one service account. It needs no project-wide roles. Linking the account to Chrome grants access to the publisher's items. The provider and service-account binding were configured on 7 September 2026 with the approved restricted trust. This repository uses GitHub immutable subject claims, including owner ID 306750767 and repository ID 1305627141. An earlier legacy-format subject binding remains pending removal; the same provider condition restricts both entries.

The main workflow first validates access with `node scripts/publish-chrome.mjs --check`. This only reads store status using the short-lived token. A green PR run is not proof of configured credentials. Activation was verified on 7 September 2026 in Chrome job https://github.com/Team-Killing-Bastards/MissionChief-Command-Nexus/actions/runs/34159666895/job/101860284928: authentication and the store status check passed; version 3.0.43.17 was already published, so no duplicate upload was sent. The independent Edge job still reported InProgressSubmission.
