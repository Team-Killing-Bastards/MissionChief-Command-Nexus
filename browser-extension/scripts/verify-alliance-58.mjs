...
if(process.argv.includes('--resume')){
 for(const row of JSON.parse(fs.readFileSync('audit/alliance58-verification-summary.json'))){
...
for(const [name,args]of steps.slice(summary.length)){
 ...
 fs.writeFileSync('audit/alliance58-verification-summary.json',JSON.stringify(summary,null,2)+'\n');
