// Local validation: Auto lifecycle, UI, responsiveness and distribution.
...
const summary=[];
// Resume only when repairing a failed harness without changing already-checked
// runtime/source. A source edit requires a fresh run without this flag.
if(process.argv.includes('--resume')){
 for(const row of JSON.parse(fs.readFileSync('audit/auto-focus-verification-summary.json'))){
  if(!row.passed||steps[summary.length]?.[0]!==row.name)break;
  summary.push(row);
 }
}
for(const [name,args]of steps.slice(summary.length)){
 const result=spawnSync(process.execPath,args,{encoding:'utf8',windowsHide:true,timeout:180000,maxBuffer:4*1024*1024});
 ...
 fs.writeFileSync('audit/auto-focus-verification-summary.json',JSON.stringify(summary,null,2)+'\n');
