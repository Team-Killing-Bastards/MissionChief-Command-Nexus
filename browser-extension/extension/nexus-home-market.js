...
(() => {
  ...
  let root,body,status,refresh,style,controller=null,epoch=0,busy=false,knownHome=null,entries=[];
  ...
  function mount(){
    ...
    const market=el('a','Full vehicle market');market.href=buildingPath+'/vehicles/new';toolbar.append(refresh,market);root.append(toolbar);
  ...
  function render(doc){
    entries=[];body.replaceChildren();const table=el('table'),head=table.createTHead().insertRow();for(const title of ['Category','Vehicle','Quick buy'])head.append(el('th',title));const tbody=table.createTBody();
    ...
    state.rows=entries.length;body.append(table);
    if(!entries.length){table.remove();status.textContent='Vehicle options could not be read. Refresh or use the full vehicle market.';return;}
  ...
  function stop(){epoch++;controller?.abort();controller=null;entries=[];busy=false;state.rows=0;state.active=false;root?.remove();style?.remove();root=body=status=refresh=style=null;}
