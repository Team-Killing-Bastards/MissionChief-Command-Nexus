...
## Current production baseline — v3.0.12

- [x] Publish one canonical userscript on trusted `main`.
- [x] Prepare Command Nexus `3.0.12` with Mission Finder `V10.6.177`, true stopped-map idle loading, a lightweight page-only B preload, reversible sustained-pressure A-only protection, exact bounded transport recovery, prisoner-handoff redirect recovery, an atomic per-mission Dispatch latch, bounded 12-hour telemetry and station-aware staffing evidence.
