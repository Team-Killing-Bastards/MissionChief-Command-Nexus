...
This document describes the architecture in the current MissionChief Command Nexus v1.1.7 production source and the direction for future consolidation.
...
The canonical module baseline is Resource Administration `V4.2.8` and Mission Finder `V10.7.5`. The Resource Administration interfaces report Unit Naming `3.3.27`, Station Naming `1.3.22` and Personnel Assignment `1.3.12`. Exact release and component-version validation belongs to `scripts/validate-userscript.mjs`; behavioral regressions do not pin these numbers.
