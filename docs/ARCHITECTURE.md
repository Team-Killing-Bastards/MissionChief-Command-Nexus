...
Mission analytics is an optional external integration, not part of dispatch readiness. It is disabled by default and exposed through one **Sharing & Sync** checkbox. The approved private Apps Script URL is compiled into the trusted two-user build and mirrored to the compatibility storage key automatically. Identity comes from MissionChief's native `#navbar_profile_link`; its numeric profile ID is authoritative, the visible username is current and the browser device ID is diagnostic only. No endpoint, user, Save, Sync, Forget, queue or status controls are rendered.

The client keeps a bounded, priority-aware local outbox and one persistent pending batch ID. The five-minute top-window timer remains a fallback, a queue of 20 events starts an eager bounded drain and each pass can send several idempotent 40-event batches. Enabling Sharing & Sync hands an immediate full backlog-drain request to the top-window owner even when the checkbox is used inside a mission frame or pop-out. Existing queues and pending batch IDs are preserved; disabling only stops recorder and upload timers. The shared lock is renewed before every batch, and one response timeout immediately reconfirms the same batch ID instead of waiting five minutes for the normal retry. Exact dispatch identities are also retained for 15 seconds across mission frames. The Apps Script backend provides the authoritative idempotency boundary under a script lock: repeated batches are acknowledged, conflicting reuse is rejected, missing unit rows from a partial prior write are repaired, and semantic dispatch retries are suppressed with their unit rows.

The transport keeps `@grant none`. It posts through a hidden iframe/form to the compiled private `script.google.com` deployment and accepts a reply only when its nonce and trusted Google origin match and the bounded response-window parent chain reaches the exact form-target iframe. This permits Apps Script's nested HtmlService sandbox without accepting unrelated Google frames. The workbook sanitizes formula-leading text; the endpoint must be rotated if this trusted private distribution is disclosed because private-URL mode deliberately has no per-device credential or revocation boundary.
