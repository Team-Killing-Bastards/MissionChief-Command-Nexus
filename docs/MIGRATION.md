...
The current Command Nexus `3.0.0` source retains versioned keys from both established engines and adds V3 pipeline/session keys for sole-owner dispatch, warm preloads, recovery and continuity. This preserves existing behaviour, but a complete formal migration matrix has not yet been proven for every combination of stored data.
