...
expect(source.includes('MISSION FINDER V10.6.159'), 'Mission Finder baseline must remain V10.6.145');
expect(source.includes("const UNIT_VERSION = '3.3.27';"), 'Unit Naming must be 3.3.11');
expect(source.includes("const STATION_VERSION = '1.3.22';"), 'Station Naming must be 1.3.5');
expect(source.includes("const PERSONNEL_VERSION = '1.3.10';"), 'Personnel Assignment must remain 1.3.10');

const moduleStart = source.indexOf("if (window.__MC_NAMING_TOOLS_V428__) return;");
const moduleEnd = source.indexOf("const UNIT_VERSION = '3.3.27';", moduleStart);
...
expect(startup.includes('const TOOL_IS_STATION_OVERVIEW_ROUTE'), 'Missing shared Stations overview route classifier');
expect(startup.includes('const TOOL_IS_STANDALONE_STATION_OVERVIEW'), 'Missing standalone Stations overview classifier');
expect(startup.includes('TOOL_IS_TOP_WINDOW &&'), 'Standalone Stations overview must require top-window ownership');
expect(startup.includes('const TOOL_IS_STATION_OVERVIEW_FRAME'), 'Missing Stations popup frame classifier');
expect(startup.includes('TOOL_IS_TOP_WINDOW ||'), 'Top window must not be classified as a child popup');
...
const standaloneGate = overview.indexOf('TOOL_IS_STANDALONE_STATION_OVERVIEW');
const popupGate = overview.indexOf('TOOL_IS_STATION_OVERVIEW_FRAME');
const desktopGate = overview.indexOf('if (!isIosSafariWebsite())');
expect(standaloneGate >= 0, 'Standalone Stations window must have an explicit overview route');
