...
for (const token of ['function mfIsExactPatientTransportAnchor(', 'function mfFindExactPatientTransportAnchor(', 'function mfFindExactPatientTransportAnchorDeep(', 'a.btn-success[href*="/patient/"]', '/^\\/vehicles\\/\\d+\\/patient\\/\\d+\\/?(?:[?#].*)?$/']) if (!source.includes(token)) fail(`missing ${token}`);
